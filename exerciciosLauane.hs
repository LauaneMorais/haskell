...
piAprox :: Int -> Double
piAprox n = 4 * piSobreQuatroAprox n

piSobreQuatroAprox :: Int -> Double
piSobreQuatroAprox n = sum [(if odd i then 1 else -1) * 1 / (2 * fromIntegral i - 1) | i <- [1..n]]
-- piSobreQuatroAprox n = sum [(if odd i then 1 else -1) * 1 / (fromIntegral i) | i <- [1..n], odd i]
